...
{    
    ...
    uint8_t spatial_attitude_head[2] = {0x55,0x51};
    uint8_t appendix_infor_head[2] = {0x55,0x50};

    esp_err_t ret = i2c_master_read_imu(I2C_MASTER_NUM, data_rd, sizeof(data_rd));

    memcpy(imu_data_rd->spatial_attitude,spatial_attitude_head,2); 
    ...
    memcpy(imu_data_rd->spatial_attitude+2,data_rd+8,6); 

    //0x37  X轴角速度：wx=((wxH<<8)|wxL)/32768*2000(°/s)
    //0x38  Y轴角速度：wy=((wyH<<8)|wyL)/32768*2000(°/s)
    //0x39  Z轴角速度：wz=((wzH<<8)|wzL)/32768*2000(°/s)
    memcpy(imu_data_rd->spatial_attitude+8,data_rd+14,6);

    //0x51  四元数Q0：Q0=((Q0H<<8)|Q0L)/32768
    //0x52  四元数Q1：Q1=((Q1H<<8)|Q1L)/32768
    memcpy(imu_data_rd->spatial_attitude+14,data_rd+66,4);

    memcpy(imu_data_rd->appendix_infor,appendix_infor_head,2);

    //0x32  分钟：MM；
    //0x32  秒：SS
    //0x33  毫秒：MS=((MSH<<8)|MSL)
    memcpy(imu_data_rd->appendix_infor+2,data_rd+4,4);

    //0x40  温度：T=((TH<<8)|TL) /100 ℃
    memcpy(imu_data_rd->appendix_infor+6,data_rd+32,2);
    ...
    memcpy(imu_data_rd->appendix_infor+8,data_rd+26,6);

    //0x53  四元数Q2：Q2=((Q2H<<8)|Q2L)/32768
    //0x54  四元数Q3：Q3=((Q3H<<8)|Q3L)/32768
    memcpy(imu_data_rd->appendix_infor+14,data_rd+70,4);
